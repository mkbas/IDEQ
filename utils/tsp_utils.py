...
def merge_tours(adj_mat, np_points, edge_index_np, sparse_graph=False, parallel_sampling=1, dmat=None, max_iter=1):
  """
  ...
  splitted_sorted_edges=[np.argsort((-np.asarray(adj_mat) / dist).flatten()).astype('int32') for adj_mat, dist in zip(splitted_adj_mat,dmat)]
  #splitted_d_mat=[i for i in dmat]
  def f(points, adj_mat,  sorted_e, dmat):
      N=int(adj_mat.shape[0])
      sorted_e=sorted_e.flatten()
      A=np.zeros_like(sorted_e, dtype=np.int32)
      m_i =lib.merge(sorted_e, N, A)
      return A.astype(np.double).reshape(N,N),m_i  
  if np_points.shape[0] > 1000 and parallel_sampling > 1:
    with Pool(parallel_sampling) as p:
      results = p.starmap(
          f,
          zip(splitted_points, splitted_adj_mat, splitted_sorted_edges, dmat),
      )
  else:
    results = [
        f(_np_points, _adj_mat, sorted_edges,dm) for _np_points, _adj_mat,sorted_edges, dm in zip(splitted_points, splitted_adj_mat, splitted_sorted_edges, dmat)
    ] 
