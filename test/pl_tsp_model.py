...
from test.pl_meta_model import COMetaModel
...
class TSPModel(COMetaModel):
  ...
  def categorical_denoise_step(self, points, xt, t, device, edge_index=None, target_t=None, np_points=None, np_e_i=None, o_e_i=None, dmat=None):
    with torch.no_grad():
      ...
      if not self.sparse:
        ...
        x0_tilde, _ = merge_tours(
          x0_pred_prob.float().cpu().detach().numpy()[..., 1].reshape(-1) + 1e-6, np_points, np_e_i,
        ...
        x0_tilde, _ = batched_two_opt_torch(
          np_points.astype("float64"), np.array(x0_tilde).astype('int64'),
        ...
        x0_pred_prob[..., 1] = torch.cat(x0_pred_list, axis=1)
  ...
  def test_step(self, batch, batch_idx, split='test'):
    ...
    dmat= [distance_matrix(i,i)+1e-6 for i in np_points.reshape(batch_size, -1,2)]
    ...
    if self.args.parallel_sampling > 1:
      if not self.sparse:
        ...
        dmat=dmat*self.args.parallel_sampling
    ...
    for _ in range(self.args.sequential_sampling):
      ...
      for i in range(steps):
        ...
        xt, xt_prob = self.categorical_denoise_step(
          points, xt, t1, device, edge_index, target_t=t2,np_points=np_points, np_e_i=np_edge_index, o_e_i=original_edge_index, dmat=dmat)
      ...
      tours, merge_iterations = merge_tours(  # [B, N+1], list
          ...
          parallel_sampling=self.args.parallel_sampling,
          dmat=dmat, 
          max_iter=self.args.two_opt_iterations,
      )
    ...
    if self.args.rewrite:
      ...
      for suff in range(self.args.rewrite_steps):
        ...
        for i in range(steps_inf):
          ...
          t1 = np.array([t1]).astype(int)
          t2 = np.array([t2]).astype(int)
          g_xt ,_= self.categorical_denoise_step(points, g_xt, t1, device, edge_index, target_t=t2,np_points=np_points, np_e_i=np_edge_index, o_e_i=original_edge_index, dmat=dmat)
    ...
    if self.args.res_file is not None:
      with open(self.args.res_file, "a+") as f:
        f.write(str(gt_cost)+" ; "+str(g_best_solved_cost)+" ; "+str(coord.shape[0]) + " \n")                
    metrics = {
